...
log = get_logger("ingest.gee.climate")


def ingest(resolution: int = 7, *, fresh: bool = False) -> int:
    if existing := should_skip("gee.climate", fresh=fresh):
        log.info("ingest.skip_recent", source="gee.climate", existing_run_id=str(existing))
        return 0
